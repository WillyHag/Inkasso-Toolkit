...
// ════════════════════════════════
  ...
  doc.text('FORLIKSBEREGNING', 105, 20, { align: 'center' });
  
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(`Generert: ${dato}`, 105, 30, { align: 'center' });
  ...
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text('KRAVETS POSTER', 20, 50);
  
  doc.setFontSize(11);
  doc.setFont('helvetica', 'normal');
  let y = 65;
  const poster = [
    ['Hovedstol', data['f-hovedstol'] || '0'],
    ['Inkassogebyr/salær', data['f-gebyr'] || '0'],
    ['Renter', data['f-renter'] || '0'],
    ['Rettslige kostnader', data['f-rettslige'] || '0']
  ];
  
  poster.forEach(([label, verdi]) => {
    doc.text(`${label}:`, 25, y);
    doc.text(kr(parseKr(verdi)), 120, y);
    y += 8;
  });
  ...
  y += 5;
  doc.setFont('helvetica', 'bold');
  doc.text('TOTALKRAV:', 25, y);
  const total = parseKr(data['f-hovedstol'] || '0') + parseKr(data['f-gebyr'] || '0') + 
                parseKr(data['f-renter'] || '0') + parseKr(data['f-rettslige'] || '0');
  doc.text(kr(total), 120, y);
  
  // Forliksandel
  y += 20;
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text('FORLIKSANDEL', 20, y);
  
  doc.setFontSize(11);
  doc.setFont('helvetica', 'normal');
  y += 15;
  const rabatter = [
    ['Rabatt på renter', data['sl-renter-val'] || '0%'],
    ['Rabatt på salær', data['sl-salar-val'] || '0%'],
    ['Rabatt på hovedstol', data['sl-hoved-val'] || '0%']
  ];
  
  rabatter.forEach(([label, verdi]) => {
    doc.text(`${label}:`, 25, y);
    doc.text(verdi, 120, y);
    y += 8;
  });
  ...
  // Hent resultater fra DOM hvis tilgjengelig
  const forlikBelop = document.getElementById('r-forlik')?.textContent || '–';
  const sparer = document.getElementById('r-sparer')?.textContent || '–';
  const sparerPct = document.getElementById('r-sparer-pst')?.textContent || '–';
  
  doc.text(`Forliksbeløp:`, 25, y);
  doc.text(forlikBelop, 120, y);
  y += 8;
  doc.text(`Skyldner sparer:`, 25, y);
  doc.text(sparer, 120, y);
  y += 8;
  doc.text(`Prosent av totalkrav:`, 25, y);
  doc.text(sparerPct, 120, y);
  
  // Betalingsinformasjon
  y += 20;
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text('BETALINGSINFORMASJON', 20, y);
  
  doc.setFontSize(11);
  doc.setFont('helvetica', 'normal');
  y += 15;
  doc.text(`Kontonummer: ${data['f-kontonummer'] || '–'}`, 25, y);
  y += 8;
  doc.text(`KID: ${data['f-kid'] || '–'}`, 25, y);
  
  // Footer
  doc.setFontSize(8);
  doc.setFont('helvetica', 'italic');
  doc.text('Generert av Forlikskalkulator', 105, 280, { align: 'center' });
